...
@if (config('app.gocpa_project') === 'cloud')
    @isset($onboarding_messages)
        @foreach ($onboarding_messages as $message)
...
<div class="row">
    <x-box class="col-lg-3">
        ...
        <div>
            <h1 class="m-0">@money($stat['orders']['gross_amount_sum'])</h1>
            <div class="stat-percent font-bold text-success float-end">{{ __('advertiser.report.not-set') }}</div>
            <small>{{ __('advertiser.report.sales-program') }}</small>
        </div>
    </x-box>
    <x-box class="col-lg-3">
    ...
    <x-box class="col-lg-3">
        <x-slot name="title">{{ __('advertiser.report.ltv') }}</x-slot>
        <x-slot name="rightblock">
            <span class="badge bg-danger small font-weight-light">{{ __('advertiser.report.for-all-time') }}</span>
        </x-slot>
        <div>
            <h1 class="m-0">@money($stat['ltv_total'])</h1>
            <div class="stat-percent font-bold text-danger float-end">@money($stat['ltv_first'])</div>
            <small>{{ __('advertiser.report.for-the-first-month') }}</small>
        </div>
    </x-box>
</div>
            ...
            <div class="col-md-4 my-auto">
                <ul class="list-unstyled">
                    <li class="my-3">
                        ...
                        @if ($week_count_all != 0)
                        <div class="progress">
                            <div class="progress-bar" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                 title="{{ __('lists.orderStateList.lead.R.plural.new') }}: {{ $week_count_new }}"
                                 style="background-color: #3a8dca; width: {{ ($week_count_new / $week_count_all) * 100 }}%; min-width: 3em;">{{ round($week_count_new / $week_count_all * 100) }}
                                %
                            </div>
                            <div class="progress-bar" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                 title="{{ __('lists.orderStateList.lead.R.plural.sale') }}: {{ $week_count_sale }}"
                                 style="background-color: #aae0d3; width: {{ ($week_count_sale / $week_count_all) * 100 }}%; min-width: 3em;">{{ round($week_count_sale / $week_count_all * 100) }}
                                %
                            </div>
                            <div class="progress-bar" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                 title="{{ __('lists.orderStateList.lead.R.plural.reject') }}: {{ $week_count_reject }}"
                                 style="background-color: #BBBBBB; width: {{ ($week_count_reject / $week_count_all) * 100 }}%; min-width: 3em;">{{ round($week_count_reject / $week_count_all * 100) }}
                                %
                            </div>
                        </div>
                        @endif
            ...
            <x-slot name="thead">
                <tr>
                    ...
                    @if($pp_target == 'products')
                        ...
                        <th>{{ __('advertiser.report.orders-amount') }}</th>
                    @else
                        <th>{{ __('advertiser.report.leads') }}</th>
                        <th>{{ __('advertiser.report.confirmed-leads') }}</th>
                    @endif
                </tr>
            </x-slot>
            <x-slot name="empty">
